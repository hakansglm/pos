...
#[CoversClass(PosNetPosQuery::class)]
#[CoversClass(AbstractPosQuery::class)]
class PosNetPosQueryTest extends TestCase
{
    private PosNetPosAccount $account;

    private PosNetPosQuery $posQuery;
    ...
    {
        $this->assertTrue(PosNetPosQuery::supports(PosNetPos::class));
        $this->assertFalse(PosNetPosQuery::supports(AkbankPos::class));
    }

    public function testIsSupportedQuery(): void
    {
        $this->assertTrue(PosNetPosQuery::isSupportedQuery(PosQueryInterface::QUERY_TYPE_CUSTOM_QUERY));
        $this->assertFalse(PosNetPosQuery::isSupportedQuery(PosQueryInterface::QUERY_TYPE_HISTORY));
        $this->assertFalse(PosNetPosQuery::isSupportedQuery(PosQueryInterface::QUERY_TYPE_INSTALLMENT_RATES));
        $this->assertFalse(PosNetPosQuery::isSupportedQuery(PosQueryInterface::QUERY_TYPE_INSTALLMENT_PRICES));
        $this->assertFalse(PosNetPosQuery::isSupportedQuery('unknown_query_type'));
    }
    ...
    public function testCustomQuery(): void
    {
        ...
        $requestData  = ['bin' => '415956', 'MerchantNo' => '6706598320'];
    ...
    private function createPosQuery(): PosNetPosQuery
    {
        return new PosNetPosQuery(
            [],
            $this->account,
            $this->requestMapperMock,
            $this->httpClientStrategyMock,
